...
public class ProcessorService {
	...
	public static Boolean calculateNearStation(Point nextOrder, List<Station> stations) {
		...
		for (Station st : stations) {
			if (DistanceCalculator.distance(nextOrder, st) <= Configuration.STATION_MAX_DISTANCE)
				return true;
		}
	...
	public static void logTraficInfo(Point currentPoint) {
		...
		System.out.println("DRONE: " + currentPoint.getDroneId() + " TIME: " + LocalTime.now() + " SPEED: "
				+ Configuration.DRONE_SPEED + " TRAFFIC: " + TraficCalculator.getDensity());
	}
